...
1. **MemFS + memory sync** — MemFS fully working (git-backed memory filesystem). Sync script (`sync-agent-memory.ps1`) upgraded with `-AllAgents` and `-SyncMemFS` flags. 60 MemFS files synced across 4 agents (Laura: 29, Drop Agent: 4, AIC Agent: 13, Transcript Agent: 14). Local clone at `C:\Users\marga\OneDrive\Documents\GitHub\golaunchmate\memories`. Dashboard at https://golaunchmate.github.io/memories/dashboard/ now shows legacy warnings for core memory blocks, MemFS file tree section with last-synced timestamps, archival-memory in separate category, and tree caching to avoid GitHub API rate limiting. Core-memory blocks and archival memory now synced for all 4 agents via `run_code_with_tools` (agent environment has LETTA_API_KEY).
...
6. **AIC Club Discord integration** — AIC agent (`agent-300f6e26`) has its own Discord bot account. Consolidation disabled; using strict channel gating instead. Laura wants the AIC Agent visually distinguished in the memory dashboard as NOT part of LaunchMate (red color coding + warning banner — it's for SDSU AI Club).
