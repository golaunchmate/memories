...
- **NEVER** use `agent_id` as a parameter name in custom tools Ã¢â‚¬â€ Letta injects it internally, causing collisions
...
- Don't post every small change Ã¢â‚¬â€ wait until tasks are complete
...
- **Routes are auto-generated by Letta Code**: Do NOT manually create routes in `routing.yaml`. Letta Code auto-generates them when channels are configured through the app. The agent was manually creating routes on July 22, 2026 and Laura corrected this Ã¢â‚¬â€ "the routes should be made automatically, not by you, right?"
...
- **Shared bot tokens work with allowlisting**: Multiple agents can share the same Discord bot token if each uses channel allowlisting (no `*` wildcard). Non-matching adapters drop messages immediately Ã¢â‚¬â€ no agent interaction, no token cost.
...
- `&&` doesn't work in PowerShell Ã¢â‚¬â€ use `;` or separate commands
- `2>/dev/null` doesn't work Ã¢â‚¬â€ use PowerShell equivalents
- Unix commands (grep, find, cat) unavailable Ã¢â‚¬â€ use PowerShell cmdlets
...
### GitHub Deployment
- Tool parameters have ~50k char limit Ã¢â‚¬â€ use `run_code_with_tools` for large content
...
- **Fix**: Cache the tree response from `loadAgents()` and reuse it in `loadAgentTree()` Ã¢â‚¬â€ never fetch the tree twice. Always check `response.ok` and handle 403 rate limit responses explicitly.
...
### Laura's Work Style
- She moves on from features when burnt out, even if imperfect Ã¢â‚¬â€ don't over-polish
...
- The `human.md` focus areas list is a snapshot, not a live feed Ã¢â‚¬â€ always verify with recent conversation history before sharing
