...
18. **LETTA_API_KEY local limitation**: The API key is only available in the Letta agent runtime (via `run_code_with_tools`), NOT in local PowerShell. The sync script (`sync-agent-memory.ps1`) can sync MemFS files locally but cannot fetch core-memory blocks without the key. Run core-memory sync from within the agent environment or set the key locally.
19. **Discord reply discipline**: When Laura messages from Discord (`channel-notification source="discord"`), the FIRST action must be `MessageChannel(action="send", channel="discord", chat_id=...)`. Never respond with plain assistant text. This failed AGAIN on July 22, 2026 — Laura said "you didn't respond to my message in discord!" This is the #1 recurring failure pattern.
