# DEPRECATED: Use sync-laura.ps1 instead — it uses the API approach (no auth prompts)
# This script used git push which triggers Windows account selection prompts.
#
# Replacement: .\sync-laura.ps1 [-SyncMemFS]
#
...
try {
    ...
    Write-Host "Letta server push failed (this is OK if pushing to GitHub works)"
}
