...
- **BEST PRACTICE:** Make one edit â explain changes â wait for user to refresh â test
...
- If the problem is obvious from Step 1 (wrong param name, wrong format) â Try using the tool correctly
- If the tool still fails or seems unreliable â Pivot to `run_code_with_tools`
...
- **CRITICAL:** Custom tools must NOT use `agent_id` as a parameter name â Letta injects this internally and it causes collisions
- **PATTERN:** The tool author picks the alternative name (e.g., `target_agent_id`, `letta_agent_id`, `which_agent`, etc.) â it varies per tool. Always CHECK the tool's actual parameter names (via `get_letta_tool` or the tool's docstring) before calling it.
- **LESSON (March 2026):** attach_letta_tool and attach_memory_block were reported as "broken" because I called them with `agent_id="..."` instead of `target_agent_id="..."`. The tools were correct all along â verified against latest Letta API docs:
  - Attach tool: `PATCH /v1/agents/{agent_id}/tools/attach/{tool_id}` â (param: `target_agent_id`)
  - Attach block: `PATCH /v1/agents/{agent_id}/core-memory/blocks/attach/{block_id}` â (param: `target_agent_id`)
...
- Avoid parameter name collisions with Letta-injected variables (don't use `agent_id` â pick a different name like `target_agent_id`, `letta_agent_id`, `which_agent`, etc.)
...
- â **WRONG:** `def my_tool(github_token: str, ...)` - Don't do this!
- â **RIGHT:** `token = os.environ.get("GITHUB_TOKEN")` - Access env vars inside the tool
...
- **WRONG:** `offset=0, offset=100, offset=200...` â returns duplicates or 500 errors
- **RIGHT:** `after=last_agent_id` â use the last agent's ID from the previous page as the cursor
...
- **MESSAGES API:** GET `/v1/agents/{agent_id}/messages` â uses `message_type` field (not `role`): `user_message`, `assistant_message`, `reasoning_message`, `tool_call_message`, `system_message`, etc. Also uses cursor-based pagination with `after=message_id`.
...
- **Pattern:** Launch 2-4 explore agents in parallel â collect results â synthesize
...
- **Don't forget:** You can run commands directly in the terminal via Bash tool â not everything needs run_code_with_tools
...
- Bash is simpler and often faster for straightforward tasks â prefer the simplest tool
...
- **RULE:** Agent catalogs, usage metrics, and dashboard data â memory blocks. Historical reflections, meeting notes, insights â archival memory