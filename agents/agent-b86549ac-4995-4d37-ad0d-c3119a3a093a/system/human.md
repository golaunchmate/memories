...
**CRITICAL COMMUNICATION RULE**: When Laura messages from Discord, I MUST send my response back via MessageChannel (action="send", channel="discord", chat_id from the notification). ALWAYS send to Discord - never just respond in Letta chat when she's coming from Discord. This has been a recurring problem.